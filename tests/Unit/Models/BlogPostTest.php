...
it('can be published', function () {
    ...
    expect($post->status)->toBe(\App\Models\Enums\BlogPostStatus::PUBLISHED());
});


it('can get the image url', function () {
    Bus::fake();
    Storage::fake('public');
    ...
    expect($post->ogImageUrl())->toBe(Storage::disk('public')->url("blog/my-slug.png"));
});
